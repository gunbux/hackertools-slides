...
\title{Hackerschool: \\Shell \& scripting}
...
\begin{frame}{Common Commands}
  \begin{itemize}
    ...
    \item \mintinline{bash}{apropos} to search manual pages
    ...
    \item \mintinline{bash}{touch} to update a file's modification time; create a file
...
\begin{frame}{Command Editing Shortcuts}
  \begin{itemize}
    ...
    \item \keys{\ctrlwin + x}\keys{\ctrlwin + e}: open line in editor
  \end{itemize}
...
\end{minted}
  ...
  \begin{itemize}
    ...
    \end{lrbox}
    \item \mintinline{bash}{#!/bin/sh} is also known as the \textbf{shebang}; it specifies the interpreter\footnote{You can use other interpreters too, e.g. \footnotesize\usebox{\mybox} for a python script.}
...
\begin{frame}[fragile]{Pipe (2/2)}
  ...
  \mintinline{bash}{cat /var/log/sys*log | grep "Sep 1" | tail}
  \begin{itemize}
    \item \mintinline{bash}{cat /var/log/sys*log} prints the system log
    \item This output is fed into \mintinline{bash}{grep Sep 1}, which looks for all entries from today.
...
\begin{frame}
  ...
  \begin{itemize}
    \item \textbf{Feedback form}: \url{https://bit.ly/2120hs1fb}
    \item \textbf{Next week's Hackerschool}:

          Introduction to Git

          Saturday, 12 February, 10am
  \end{itemize}
\end{frame}
